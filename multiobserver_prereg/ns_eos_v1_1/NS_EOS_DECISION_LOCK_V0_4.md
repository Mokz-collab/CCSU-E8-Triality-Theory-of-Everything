# CCSU-MO-NS-EOS-001 — Decision Lock v0.4

**Status:** development lock; not confirmatory  
**Purpose:** preserve the six Domain Lock decisions while replacing the
externally rejected endpoint-only inner-crust bridge
...
input/provenance blockers are closed, and local thermodynamic shape validation
of the revised connector has passed on a calibration/holdout split. The
remaining blockers are evidential and implementation-specific: test every
outer-crust/χEFT pairing and quantify radius/tidal impact, obtain independent
review of proposal ranges, cross-validate TOV/Love solvers, calibrate P1–P4
thresholds out of grid, and derive the final power budget.
