...
- a thermodynamically exact shared inner-crust connector;
...
Validate the active decision lock and the two χEFT members with:

```bash
PYTHONPATH=src python -m ccsu_multiobserver.ns_eos_decisions \
  ns_eos_v1_1/ns_eos_decisions_v0_3.yaml
...
`ns_eos_decisions_v0_3.yaml` are closed.
