...
\begin{align}
&\underbrace{(u/d)\times\text{lepton/neutrino}}_{\text{4 types}}
\times \underbrace{(L/R)}_{\times\,2}
\times \underbrace{\text{color (3 for quarks, 1 for leptons)}}_{\text{avg }2}
\\[-2pt]
&\qquad\times \underbrace{\text{particle/antiparticle}}_{\times\,2}
\times \underbrace{24\ \text{vertices}}_{\text{per type}}
\ \Longrightarrow\ \mathbf{192}\ \text{fermionic states}.
\end{align}
...
\section{Embedding}
Let $E_8\supset \mathfrak{so}(16)\supset \mathfrak{so}(8)\oplus\mathfrak{so}(8)$ with the triality automorphism $\mathcal{T}$ on $\mathrm{Spin}(8)$ interchanging $(8_v,8_s,8_c)$
\[
...
\begin{align}
\log m_i^{\mathrm{(pred)}} &=
\log m_0 + n_i \log r + \kappa\langle \lambda_i, u\rangle \notag\\
&\quad +\, \eta_W \langle \lambda_i, w\rangle
      + \eta_Z \langle \lambda_i, z\rangle
      + \eta_h \langle \lambda_i, h\rangle . 
\label{eq:e8-alignment}
\end{align}
