% --- chapters/predictions.tex ---------------------------------------------
% This label attaches to the chapter, because name.tex calls \chapter{...} 
% immediately before \input{chapters/predictions.tex}


\section*{Executive Summary}
CCSU yields concrete, near–term tests in three arenas:
\begin{enumerate}[nosep]
  \item gravitational-wave ringdowns (dispersion and mode-energy partition),
  \item flavor-sector phases (CKM/PMNS) under \(\Lambda\)--mode deformations,
  \item fermion mass ladders anchored by \(\{W,Z,h,t\}\) alignment.
\end{enumerate}
...
Let \(\Xi(t)=\langle|\psi_{\rm echo}|^2-|\psi_{\rm anti}|^2\rangle_t\) be the local echo–imbalance (\label{chap:gravity}). For the 220 mode we predict a small fractional shift
\begin{equation}
\frac{\delta\omega_{220}}{\omega_{220}}
\;=\;\alpha_\Lambda\,\overline{\Xi}\;+\;O\!\left(\overline{\Xi}^{\,2}\right),
...
\subsection{\texorpdfstring{$221/220$}{221/220} frequency ratio and energy fraction}
The 221 frequency clusters near a fixed multiple of 220 under CCSU vortex pairing:
\begin{equation}
...
With the \(\Lambda\)--mode lepton warp tuned \ref{chap:flavor}), we predict near–maximal leptonic phase:
\begin{equation}
...
\end{equation}
\paragraph{Test.} Refit with/without \(\Lambda\)--mode deformations; \(\Delta\theta^q\) must remain subpercent.

\section{Fermion Mass Ladders}

\subsection{\texorpdfstring{Harmonic shells with $\{W,Z,h,t\}$ alignment}{Harmonic shells with \{W,Z,h,t\} alignment}}
...
\begin{itemize}
  \item GW stacks: median \(f_{221}/f_{220}\in[1.45,1.60]\) and positive \(\mathrm{corr}(a,\varepsilon_{221})\) (\(p<0.05\)).
